...
std::future<void> Sample3DSceneRenderer::CreateDeviceDependentResources(void)
{
	...
	for(int f_XY = 0; f_XY < Cube::g_BicycleContainer[0]->m_ADMAX; f_XY++)
		{
		std::shared_ptr<Cube::CubeKEY> f_KEY = std::make_shared<Cube::CubeKEY>();
...
void Sample3DSceneRenderer::acSaveCompletePublish_GenHesh_Upper_Reflection(void)
{
	...
	while(g_Compressing == 1)
			...
			if((g_LowtonOn == true) && ((g_classLMap->m_CPS_Result[f_OCSCP_ThreadIndex] == CPS_SCAN) && (g_Result == false)))
				...
				while(f_SizeMin < f_SoLowtonBicycleSize)
					...
					if(f_SizeMin >= f_Hesh_Cade->vec_Bicycle.size())
						...
						f_Collection->m_vec_Element[f_SizeMin]->acSync_fromBicycle(f_Bike);
		...
		int f_SizeTotal = f_SizeMin + f_ElementOnce;
		
		int f_ElementsRemaining = g_classLMap->m_ADMAX - f_SizeTotal;

		if(f_ElementsRemaining < f_ElementsOnceMax)
			{
			f_ElementOnce = f_ElementsRemaining;
			
			f_SizeTotal = f_SizeMin + f_ElementOnce;
			}

		if(f_ElementOnce > 0)
			{
			for(int f_Int = f_SizeMin; f_Int < f_SizeTotal; f_Int++)
				{
...
#if 0
			g_ElementCount += f_SizeTotal - f_SizeMin;
#else
		 ...
		 // HESH   //
		// Target //
		bool f_LoadBool = g_classLMap->ac_CalcLoadFinish();

		if((g_classLMap->m_CPS_Result.size() >= g_SaveComplete_ThreadCount) && (f_LoadBool == true))
			{
			if((g_classLMap->m_CPS_Result[f_OCSCP_ThreadIndex] == CPS_SCAN) && (g_Result == false))
				{
				int f_AttackIndex = 0;

				f_AttackIndex = f_SizeMin;
				...
				if(f_IntegerResult > 0)
					...
					if(g_Compressing == 1)
						...
						if(g_Lowton->m_adIndex.size() > 50)
							...
							{
							g_BicycleContainer[0]->m_ADMAX = 50;
							g_BicycleContainer[0]->m_ADHIGH = 50;
							g_BicycleContainer[0]->m_ADLOW = 50;
							}
...
void Sample3DSceneRenderer::acGen_GenHesh(void)
{
	...
	ag_StatusLive(f_ref_CurrentPath, "f_Hesh_Cade");
	std::shared_ptr<Cube::CubeHESH> f_Hesh_Cade = std::make_shared_reflect<Cube::CubeHESH>(Cube::g_BicycleContainer[0]->m_ADMAX, Cube::g_BicycleContainer[0]->m_ADHIGH, Cube::g_BicycleContainer[0]->m_ADLOW);
	ag_StatusLive(f_ref_CurrentPath, "f_Hash");
