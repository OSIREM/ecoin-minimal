...
void Sample3DSceneRenderer::acSaveCompletePublish_GenHesh_Upper_Reflection(void)
{
...
#else
	std::shared_ptr<Cube::CubeHESH> f_Hesh_Cade = std::make_shared<Cube::CubeHESH>(g_classLMap->m_ADMAX, g_classLMap->m_ADHIGH, g_classLMap->m_ADLOW, g_CountLM);
	...
	while(g_Compressing == 1)
		...
		if(f_Collection->m_vec_Element.size() > 0)
			{
			int f_SizeMin = f_Collection->m_vec_Element.size() - 1;
...
#if 0
			g_ElementCount += f_PossibleTarget - f_SizeMin;
#else
		...
		if(g_classLMap->m_LoadFinish == false)
			{
			f_LoadInt = g_classLMap->ac_CalcLoadFinish();
			}
		else
			{
			f_LoadInt = 1;
		...
		if((g_classLMap->m_CPS_Result.size() >= g_SaveComplete_ThreadCount) && (f_LoadInt == 1))
			{
					...
					if(g_Compressing == 1)
						...
						for(int f_Jet = g_Lowton->vec_Bicycle.size(); f_Jet < f_OC_Mark_Count; f_Jet++)
							...
							if(f_Jet < f_Hesh_Cade->vec_Bicycle.size())
								...
								::sprintf(g_cubevalue[48], "%i", g_JobID);
								...
								::sprintf(g_cubevalue[54], "%f", (float)ECOIN_TARGET_SCP_COMPARE_THRESH);
						...
						if(f_OC_Lowton_Mark_Count >= g_classLMap->m_ADMAX) //complete work, winning conditions!
							...
							//toss share calculation transaction onto next block/jobid
							ag_addGenesisTracking(f_SCP_ShareID);
							...
							std::string f_TXJob_InputHash = f_job;

							g_hasha.init(); //reset hasher state
							g_hasha.process(f_TXJob_InputHash.begin(), f_TXJob_InputHash.end());
							g_hasha.finish();
							picosha2::get_hash_hex_string(g_hasha, g_LowtonHash->m_OutputHash);

							//job-hash
							std::string f_TXJob_OutputHash = g_LowtonHash->m_OutputHash;
							...
							//share-hash
							std::string f_Share_OutputHash = g_LowtonHash->m_OutputHash;

							g_Lowton->m_Hash = g_LowtonHash;

							char* g_sharefield_update[5];
							char* g_sharevalueup[5];
							char* f_sharewhere[1];
							...
							::sprintf(g_sharefield_update[0], "shareledger");
							...
							::sprintf(g_sharevalueup[0], "%s", f_Share_OutputHash.c_str());
							::sprintf(g_sharevalueup[1], "%s", f_TXJob_OutputHash.c_str());
							::sprintf(g_sharevalueup[2], "%f", g_Lowton->m_bckred);
							::sprintf(g_sharevalueup[3], "%f", g_Lowton->m_bckgreen);
							::sprintf(g_sharevalueup[4], "%f", g_Lowton->m_bckblue);

							sprintf(f_sharewhere[0], "id = %i", f_SCP_ShareID);

							g_SafeL[0]->Update("share", (const char **)g_sharefield_update, (const char **)g_sharevalueup, 5, f_sharewhere[0], false, false);
							...
							ag_StatusLive(f_ref_CurrentPath, "f_Ack", true);
							std::shared_ptr<Ackent> f_Ack = std::make_shared_reflect<Ackent>(ACK_TYPE_SHARE, g_ShareID, 1/*f_PEERCNT*/);
...
{
	std::string f_ref_CurrentPath = "Sample3DSceneRenderer::acSaveCompletePublish_GenHesh_Upper";
	bool f_OCP_LOCK_ON = false;
...
void Sample3DSceneRenderer::acGen_GenHesh(void)
{
	...
	switch(g_Type)
		...
		case 1:
			...
			if(f_Target_1 == nullptr)
				{
				ag_StatusLive(f_ref_CurrentPath, "f_Target_1");
		...
		case 2:
			...
			if(f_Target_2 == nullptr)
				{
				ag_StatusLive(f_ref_CurrentPath, "f_Target_2");
...
#else
	std::shared_ptr<Cube::CubeHESH> f_Hesh_Cade = std::make_shared<Cube::CubeHESH>(g_clag_BicycleContainer[0]ssLMap->m_ADMAX, g_BicycleContainer[0]->m_ADHIGH, g_BicycleContainer[0]->m_ADLOW, g_CountLM);
	...
	while(g_Viewing == 0)
		...
		if(g_KeepShare == false)
			...
			int f_Diff_admax = f_Hesh_Cade->m_ADHIGH - f_Hesh_Cade->m_ADLOW;

			float f_NewADMAX_float = (float)f_Hesh_Cade->m_ADLOW + ((float)f_Diff_admax * f_Finger);

			f_NewADMAX = (int)f_NewADMAX_float;

			if(f_NewADMAX > f_Hesh_Cade->m_ADMAX)
				{
				f_NewADMAX = f_Hesh_Cade->m_ADMAX;
				}

			while(f_Hesh_Cade->m_adIndex.size() > f_NewADMAX)
				{
				f_Hesh_Cade->vec_Bicycle.pop_back();
				f_Hesh_Cade->m_adIndex.pop_back();
				f_Collection->m_vec_Element.pop_back();
			...
			switch(g_Type)
				...
				case 1:
					...
					f_Result = g_Target_1->acCompare(f_Target_1, 80.55f, f_Difficulty, true);
					...
					f_Target_1->m_Mark = (f_MatchMark * ECOIN_TARGET_MATCH_MIXIN) * f_Target_1->m_Mark;
					}break;
				...
				case 2:
					...
					f_Result = g_Target_2->acCompare(f_Target_2, 80.55f, f_Difficulty, true);
					...
					f_Target_2->m_Mark = (f_MatchMark * ECOIN_TARGET_MATCH_MIXIN) * f_Target_2->m_Mark;
					}break;
		...
		if((f_Result == true) && (g_KeepShare == false))
			...
			char* g_sharevalue[21];

			for(int f_Count = 0; f_Count < 21; f_Count++)
				{
			...
			::sprintf(g_sharevalue[7], "%f", f_Difficulty);
			 ...
			 // Hash
			//
			f_Hesh_Cade->m_Hash = std::make_shared<Cube::CubeHASH_originstring>();
			...
			g_hasha.process(f_InputHash.begin(), f_InputHash.end());
			g_hasha.finish();
			picosha2::get_hash_hex_string(g_hasha, f_Hesh_Cade->m_Hash->m_OutputHash);
#else
			...
			sprintf(g_sharevalue[10], "%s", f_Hesh_Cade->m_Hash->m_OutputHash.c_str());
			sprintf(g_sharevalue[11], "%s", f_Hesh_Cade->m_Hash->m_OutputHash.c_str());
			...
			sprintf(g_sharevalue[16], "%f", 50.0f);
			sprintf(g_sharevalue[17], "%f", 0.0f);
			sprintf(g_sharevalue[18], "%f", 0.0f);
			sprintf(g_sharevalue[19], "%f", 0.0f);
			sprintf(g_sharevalue[20], "%i", -1);

			//ECN::Hesh...Sending BlockHesh.

			char shlquery[1000];
			sprintf(shlquery, "SELECT id FROM share WHERE shareledger = '%s'", f_Hesh_Cade->m_Hash->m_OutputHash.c_str());
			...
			if(f_ShareLCount == 0)
				{
				int f_ShareUniqueID = g_SafeL[0]->Insert("share", (const char **)g_sharefield, (const char **)g_sharevalue, 21, 2, false);
				...
				std::shared_ptr<Ackent> f_Ack = std::make_shared_reflect<Ackent>(ACK_TYPE_SHARE, g_ShareID, f_PEERCNT);
				f_Ack->m_ECN = 0.0f;
				f_Ack->m_Reward = 50.0f;
				f_Ack->m_Origin = 1;
				f_Ack->m_Owner = g_Wallet->GetAdr();
			...
			if((f_Mark < g_LowtonMark) && (g_KeepShare == false))
				...
				g_LowtonHash = f_Hesh_Cade->m_Hash;
			...
			if(g_currenttime > g_storagetime)
				...
				if(f_ShareLCount == 0)
					...
					char* f_idquery2 = Cube::ag_ClearChar(150);
...
void Sample3DSceneRenderer::acGen_GenHesh(void)
{
	std::shared_ptr<Cube::CubeHESH> f_Hesh_Cade = std::make_shared_reflect<Cube::CubeHESH>(Cube::g_BicycleContainer[0]->m_ADMAX, g_ADHIGH, g_ADLOW);
	std::shared_ptr<Cube::CubeHASH_originstring> f_Hash = std::make_shared_reflect<Cube::CubeHASH_originstring>();
	...
	if(g_KeepShare == false)
		...
		switch(g_Type)
			...
			case 1:
				{
				std::shared_ptr<classTarget> f_Target = std::make_shared_reflect<classTarget>(f_Hesh_Cade);
	...
	if((f_Result == true) && (g_KeepShare == false))
		...
		sprintf(g_sharevalue[17], "%f", 0.5f);
		sprintf(g_sharevalue[18], "%f", 0.4f);
		sprintf(g_sharevalue[19], "%f", 0.1f);
		sprintf(g_sharevalue[20], "%i", 0);
