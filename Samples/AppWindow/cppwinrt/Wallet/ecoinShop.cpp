...
{
/// <summary>
/// Supa-Type Generating
/// </summary>

int g_Type = 0;
string g_TypeName = "";

shared_ptr<eshop> g_ShareShop;
...
eshop::eshop()
{
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClear", (ecom_base::ControllerMethod_void)&eshop::acClear);

	//global referencing
	ag_Push(g_Type, f_ref_CurrentPath + g_ref_global_Div + "g_Type");
	ag_Push(g_TypeName, f_ref_CurrentPath + g_ref_global_Div + "g_TypeName");
	ag_Push(g_ShopPage, f_ref_CurrentPath + g_ref_global_Div + "g_ShopPage");
#endif
}
		...
		if(f_DescriptionLength == 0)
			...
			if(f_PrefixLength == 0)
				...
				if(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].at(0) == '0')
					...
					{
					f_ShopLCount = g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->size();
					}
...
int eshop::acPopulate(void)
{
	...
	if(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].at(0) == '0')
		...
		{
		f_ConLCount = g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->size();
		}
	...
	if(f_ConLCount > 0)
		...
		for(int f_XY = 0; f_XY < f_ConLCount; f_XY++)
			...
			f_Product->m_ShareID = atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_XY)[0].c_str());
			f_Product->m_Score = atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_XY)[1].c_str());
			f_Product->m_CostCPS = atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_XY)[3].c_str());
			f_Product->m_ComID = -1; //atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_XY][4].c_str());
			//...

			std::string f_ESL_Code = g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_XY)[2];
