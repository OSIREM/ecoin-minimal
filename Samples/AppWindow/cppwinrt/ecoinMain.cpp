	...
	m_pointerLocationX(0.0f), m_Rendering(false)
{
	...
	ag_Push(g_Viewing, f_ref_CurrentPath + g_ref_global_Div + "g_Viewing");
	...
	ag_Push(g_Compressing, f_ref_CurrentPath + g_ref_global_Div + "g_Compressing");
...
void ecoinMain::RenderLoop(void)
{
	...
	int f_OwnPeerIndex = f_ShareContainer->m_espo_OwnPeerIndex;
	g_ShareCount = f_ShareCounting;

	g_Schedular[0]->m_MainMutex.unlock();

	  //////////////////////////////
	 // Load current First Share //
	//////////////////////////////
#if 0
	g_swapChainPanel.Dispatcher().RunAsync(winrt::Windows::UI::Core::CoreDispatcherPriority::Normal, [this]()
		{
		g_PrevButton = g_swapChainPanel.FindName(StrUsingStr("prevshareButton")).as<winrt::Windows::UI::Xaml::Controls::Button>();

		g_PrevButton.Command().Execute(g_PrevButton.CommandParameter());
		});
#endif
			...
			if(g_Compressing == 0)
				...
				while(m_sceneRenderer->m_cnt_SCP_Thread < g_SaveComplete_ThreadCount)
					...
					if(g_LowtonOn == false)
						...
						if(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].at(0) != '0')
							{
							g_ShareID = atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].c_str());

							Cube::BiVector f_BCK(atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[1].c_str()), atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[2].c_str()), atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[3].c_str()));
							...
							if(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[0].at(0) == '0')
								...
								f_CountLM = atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(0)[52].c_str());
								}
							...
							for(int f_Int = 0; f_Int < f_adCount; f_Int++)
								...
								for(int f_Int2 = 0; f_Int2 < 8; f_Int2++)
									...
									std::shared_ptr<Cube::BiVector> f_Vector = ::std::make_shared_reflect<Cube::BiVector>((float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_Int)[1 + (f_Int2 * 3)].c_str()), (float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_Int)[1 + ((f_Int2 * 3) + 1)].c_str()), (float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_Int)[1 + ((f_Int2 * 3) + 2)].c_str()));
								...
								for(int f_Int3 = 0; f_Int3 < 8; f_Int3++)
									...
									std::shared_ptr<Cube::BiVector> f_Color = ::std::make_shared_reflect<Cube::BiVector>((float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_Int)[25 + (f_Int3 * 3)].c_str()), (float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_Int)[25 + ((f_Int3 * 3) + 1)].c_str()), (float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_Int)[25 + ((f_Int3 * 3) + 2)].c_str()));
							...
							ag_FreshOn<int>(g_Wallet->m_vec_mark, m_IDX, ((float)g_BicycleContainer[0]->m_ADMAX / (float)g_classLMap->m_ADMAX));
