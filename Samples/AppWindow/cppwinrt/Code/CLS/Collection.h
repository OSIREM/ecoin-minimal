...
namespace ecoin
{
	...
	public:
		classCollection(std::shared_ptr<Cube::CubeHESH> f_Hesh) : m_Type(Ground), m_Status(ELEM_Status_Birth)
			...
			ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acGen_Humanoid", (ecom_base::ControllerMethod_void)&classCollection::acClear);
#endif
			};
	...
	public:
		classCollectionScript(uint f_Type) : m_Type(f_Type), m_Status(ELEM_Status_Birth)
			...
			ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acGen_Humanoid", (ecom_base::ControllerMethod_void)&classCollectionScript::acClear);
#endif
			};
