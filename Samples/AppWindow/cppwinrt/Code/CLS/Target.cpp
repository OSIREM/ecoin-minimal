...
classTarget::classTarget(std::shared_ptr<CubeHESH> f_Hesh)
{
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acGen_Humanoid", (ecom_base::ControllerMethod_void)&classTarget::acGen_Humanoid);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acGen_Vehicle_Ground", (ecom_base::ControllerMethod_void)&classTarget::acGen_Vehicle_Ground);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acPrintStats", (ecom_base::ControllerMethod_void)&classTarget::acPrintStats);
#endif
...
classTargetScript::classTargetScript(std::shared_ptr<CubeHESH> f_Hesh)
{
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "ag_PrintScriptHeader", (ecom_base::ControllerMethod_void)&classTargetScript::ag_PrintScriptHeader);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acPrint", (ecom_base::ControllerMethod_void)&classTargetScript::acPrint);
#endif
...
void classTarget::acLoad_from_TargetID(int f_TargetID)
{
	...
	if(f_TargetID > 0)
		...
		for(int f_EleID = 0; f_EleID < f_eleCount; f_EleID++)
			...
			std::shared_ptr<classElement> f_NewElement = std::make_shared_reflect<classElement>((float)atof(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_EleID)[1].c_str()));
		...
		for(int f_ElindexID = 0; f_ElindexID < f_elindexCount; f_ElindexID++)
			{
			shared_ptr<classElement> f_Element = m_Collection->m_vec_Element[atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_ElindexID)[0].c_str())];
			shared_ptr<classElement> f_AElement = m_Collection->m_vec_Element[atoi(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)]->at(f_ElindexID)[1].c_str())];
...
classTarget_Ecom::classTarget_Ecom()
{
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acResetLine", (ecom_base::ControllerMethod_void)&classTarget_Ecom::acResetLine);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acPrepare_Lense", (ecom_base::ControllerMethod_void)&classTarget_Ecom::acPrepare_Lense);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acResetLine", (ecom_base::ControllerMethod_void)&classTarget_Ecom::acResetLine);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acPrepare_Lense", (ecom_base::ControllerMethod_void)&classTarget_Ecom::acPrepare_Lense);
#endif
}
...
classLMap::classLMap()
{
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acResetLine", (ecom_base::ControllerMethod_void)&classLMap::acResetLine);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acRangeAD", (ecom_base::ControllerMethod_void)&classLMap::acRangeAD);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClear", (ecom_base::ControllerMethod_void)&classLMap::acClear);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClearAway", (ecom_base::ControllerMethod_void)&classLMap::acClearAway);
...
#if 0
	if(m_LMap->m_vec_Measure.size() >= 142)
		{
		__debugbreak();
		}
#endif
							...
							if(ac_takeMeasurementBLTV1(f_ElementStartAD, f_ElementEndAD, f_ElementMax, f_Depth, 1) == true)	//arg1 has value
								...
								if(ac_takeMeasurementBLTV1(f_ElementStartAD, f_ElementEndAD, f_ElementMax, f_Depth, 1) == true)	//arg1 has value
									...
									if(ac_takeMeasurementBLTV1(f_ElementStartAD, f_ElementEndAD, f_ElementMax, f_Depth, 1) == true)	//value is float
										{
										char* f_Chr = ag_ClearChar(512);

										::sprintf(f_Chr, "%f&", ac_takeMeasurementFLTV1(-1, -1, -1, -1));

										f_Var1NameFA += f_Chr;

										f_Var1NameFA.push_back('%');
...
#if 0
	std::shared_ptr<Qcom_eslfunction> f_func = std::make_shared_reflect<Qcom_eslfunction>();
#else
...
classMark::classMark()
{
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acReset", (ecom_base::ControllerMethod_void)&classMark::acReset);
#endif
}
...
std::string classTarget_Ecom::ac_takeMeasurement(int f_ElementStartAD, int f_ElementEndAD, int f_ElementMax, int f_Depth)
{
	...
	while(f_ScanMeasure == true)
		...
		if((f_CharValue >= 0) && (f_CharValue <= 9))
			{
			f_CharAry.push_back((char)('0' + f_CharValue));
			}
			...
			f_CharAry.push_back((char)('A' + f_Value));
			}
...
std::string classLMap::ac_recordMeasurement(std::string f_RecordResult, int f_ElementID, int f_StartAD, int f_EndAD, int f_ElementMax, int f_Depth)
{
	...
	if(f_RecordResult.length() == 0)
		...
		ac_recordMeasurementINTV1(0, f_EcoinSpecMax, 0, 3, 4, 17, f_ElementID);
