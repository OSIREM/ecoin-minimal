...
Variable::Variable() : MuType(MuTYPE_Variable), m_MxName("EmptyName"), m_CodeType(MuCode_Var), m_PrecType(MuVar_Full), m_Variable(ECN_VAR)
{
	acClear();
};

Variable::Variable(std::string f_Stg, uint f_CodeType) : MuType(MuTYPE_Variable), m_MxName(f_Stg), m_CodeType(f_CodeType), m_PrecType(MuVar_Full), m_Variable(ECN_VAR)
{
	acClear();
...
Variable::Variable(uint f_PrecType) : MuType(MuTYPE_Variable), m_MxName("IdentVar"), m_CodeType(0), m_PrecType(f_PrecType), m_Variable(agFind_Variable_Type(f_PrecType))
{
	acClear();
...
Variable::Variable(std::string f_Stg, uint f_CodeType, uint f_PrecType) : MuType(MuTYPE_Variable), m_MxName(f_Stg), m_CodeType(f_CodeType), m_PrecType(f_PrecType), m_Variable(agFind_Variable_Type(f_PrecType))
{
	acClear();
...
int agFind_Variable_Type(int f_CurrentSpec)
	{
	if(f_CurrentSpec == ECN_FLOAT)
		{
		return ECN_VAR_FLOAT;
		}
	else if(f_CurrentSpec == ECN_INT)
		{
		return ECN_VAR_INT;
		}
	else if(f_CurrentSpec == ECN_STRING)
		{
...
std::shared_ptr<Variable> av_Identity(uint f_PrecType)
	{
	std::string f_ref_CurrentPath = "av_Identity";

	ag_StatusLive(f_ref_CurrentPath, "return Variable");
