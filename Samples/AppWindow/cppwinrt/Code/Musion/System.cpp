...
namespace ecoin
{
	...
	System::System()
		...
		Pos = std::make_shared_reflect<Variable>("Pos", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Posx");
		Posx = std::make_shared_reflect<Variable>("Posx", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Posy");
		Posy = std::make_shared_reflect<Variable>("Posy", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Posz");
		Posz = std::make_shared_reflect<Variable>("Posz", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Color");
		Color = std::make_shared_reflect<Variable>("Color", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Matrix1");
		Matrix1 = std::make_shared_reflect<Variable>("Matrix1", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Matrix2");
		Matrix2 = std::make_shared_reflect<Variable>("Matrix2", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Matrix3");
		Matrix3 = std::make_shared_reflect<Variable>("Matrix3", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "Matrix4");
		Matrix4 = std::make_shared_reflect<Variable>("Matrix4", MuCode_System);
		ag_StatusLive(f_ref_CurrentPath, "nonce");
		nonce = std::make_shared_reflect<Variable>("nonce", MuCode_System);
#else
