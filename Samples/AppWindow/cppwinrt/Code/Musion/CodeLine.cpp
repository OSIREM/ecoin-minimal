...
CodeLine::CodeLine(std::string f_Line, uint f_CHK, System* f_System)
	{   ////////////////
	 ...
	 //
	//
	std::string f_ref_CurrentPath = "CodeLine::CodeLine";
	acClear();
	m_CodeString = f_Line;
	m_Chk = f_CHK;
	m_System = f_System;
	...
	if(m_Chk < f_Size - 6)
		{
		//ag_StatusLive(f_ref_CurrentPath, "f_CodeB");
		...
		if(f_Scan)
			...
			while(f_Scan == true)
				...
				if(m_Chk < f_Line.size() - 6)
					{
					//ag_StatusLive(f_ref_CurrentPath, "f_CodeA");
		...
		if((f_Code->m_Code_DigitB != '/') &&
			...
			switch(f_Code->m_Code)
						...
						if(f_CodeSize == 3)
							...
							if(f_CodePositionOne->m_Code == MuCode_Condition)
								...
								if(f_Code->m_Number.size() > 0)
									...
									{
									//ag_StatusLive(f_ref_CurrentPath, "m_vec_Variable");
								...
								if(f_CodePositionOne->m_Code == MuCode_Function)
									...
									if(f_Code->m_Number.size() > 0)
										...
										{
										//ag_StatusLive(f_ref_CurrentPath, "m_Result_V");
				...
				case MuCode_Operator:
					...
					//ag_StatusLive(f_ref_CurrentPath, "f_Operator");
					...
					if(f_LtCode->m_vec_NameCall.size() == 1)
						...
						if(f_L_StrCode.compare("Sys") == 0)
							...
							{
							//ag_StatusLive(f_ref_CurrentPath, "f_Operator->leftHand");
					...
					if(f_RtCode->m_vec_NameCall.size() == 1)
						...
						if(f_R_StrCode.compare("Sys") == 0)
							...
							if(f_RtCode->m_Number.size() >= 1)
								...
								{
								//ag_StatusLive(f_ref_CurrentPath, "f_Operator->rightHand");
								f_Operator->rightHand = std::make_shared<Variable>(f_RtCode->m_Name[0]->m_MxName, f_RtCode->m_Code);
								m_vec_Variable.push_back(f_Operator->rightHand);
								}
							}
...
std::shared_ptr<Variable> Code::acCodeMap_Var(std::string f_MappingString, std::string f_TypeName)
{
	std::string f_ref_CurrentPath = "Code::acCodeMap_Var";

#ifdef ECOIN_OSI_MEMORY
	if(g_vec_ecomRefBase.find(f_MappingString) != g_vec_ecomRefBase.end())
		{
		std::shared_ptr<ecom_base> f_RefBasePtr = g_vec_ecomRefBase[f_MappingString];

		//ag_StatusLive(f_ref_CurrentPath, "f_MxVar");
		std::shared_ptr<Variable> f_MxVar = std::make_shared<Variable>("FreshMapVar", MuCode_Var, MuVar_Full, ECN_CLASS_VAR);

		//f_MxVar->m_Var = f_RefBasePtr->ac_Get(f_MappingString, f_TypeName);
...
std::shared_ptr<ClassVar> CodeLine::ac_Execute(std::shared_ptr<Function> f_Function)
{
	...
	//
	//
	std::string f_ref_CurrentPath = g_ref_flow_reset + "CodeLine::ac_Execute";
	...
	if(m_CodeLine == MuLine_Assign_Opr)
		...
		if(m_vec_Code[0]->m_Code == MuCode_Function)
			...
			if((m_vec_Code[0]->m_Type[0]->m_VarType >= MuFunc_Custom) && (m_vec_Code[0]->m_Type[0]->m_VarType < MuSetGLOBAL_))
				{
				std::shared_ptr<Function> f_FunctionB = m_vec_Code[0]->m_FunctionLink;

				f_FunctionB->acExecute();

				m_Result_V = f_FunctionB->m_Return_V;
				m_Result_CV = f_FunctionB->m_Return_CV;
				}
			else
				{
			...
			if((m_vec_Code[1]->m_Type[0]->m_VarType >= MuFunc_Custom) && (m_vec_Code[1]->m_Type[1]->m_VarType < MuSetGLOBAL_))
				{
				std::shared_ptr<Function> f_FunctionB = m_vec_Code[1]->m_FunctionLink;

				std::shared_ptr<Variable> f_Var = f_FunctionB->acExecute();
...
#pragma message("Warning add debugability messages according to m_Type = result")
		...
		//ag_StatusLive(f_ref_CurrentPath, "f_ClassVar");
