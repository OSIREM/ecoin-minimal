...
{
	printf("New ESL Compiler Script...\n");
}
...
std::shared_ptr<CodeLine> Contract::acScan_LineDetect(std::string f_Line)
{
	...
	while(m_Chk < f_Line.length() - 6)
		{
		//ag_StatusLive(f_ref_CurrentPath, "f_CodeLine");
		...
		if(f_CodeLine->m_CodeLine < 3)
			...
			//ag_StatusLive(f_ref_CurrentPath, "f_Condition");
			...
			//ag_StatusLive(f_ref_CurrentPath, "f_Condition->leftHand");
			...
			if(f_RtCode->m_Number.size() >= 1)
				...
				{
				//ag_StatusLive(f_ref_CurrentPath, "f_Condition->rightHand");
				...
				if(g_Function.size() >= 1 && g_Condition.size() >= 1)
					...
					if(f_Code->m_Name.size() >= 2)
						...
						for(int f_XYZ = 0; f_XYZ < g_Condition.size(); f_XYZ++)
							{
							if(g_Condition[f_XYZ]->m_MxName.compare(f_Code->m_Name[1]->m_MxName) == 0)
								{
			...
			//ag_StatusLive(f_ref_CurrentPath, "m_Frame_Function");
			...
			switch(f_CodeLine->m_CodeLine)
				...
				case MuLine_Function:
					...
					if(f_CodeLine->m_Result_V != nullptr)
						...
						for(int f_XYZ = 0; f_XYZ < g_Function[f_CodeLine->m_vec_Code[1]->m_ContractID]->m_vec_Variable.size(); f_XYZ++)
							{
							g_Function[f_CodeLine->m_vec_Code[1]->m_ContractID]->m_vec_Variable[f_XYZ] = ac_VarByName(g_Function[f_CodeLine->m_vec_Code[1]->m_ContractID]->m_vec_Variable[f_XYZ]);
							}
						}
						...
						for(int f_XYZ = 0; f_XYZ < g_Function[f_CodeLine->m_vec_Code[0]->m_ContractID]->m_vec_Variable.size(); f_XYZ++)
							{
							g_Function[f_CodeLine->m_vec_Code[0]->m_ContractID]->m_vec_Variable[f_XYZ] = ac_VarByName(g_Function[f_CodeLine->m_vec_Code[0]->m_ContractID]->m_vec_Variable[f_XYZ]);
							}
						}
					}break;
				}
