...
qamEvent::qamEvent(int f_ID) : qamType(MuTYPE_Code)
{
	...
	if(f_Int)
		...
		if(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size())
			{
			m_TriggerID = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[0].c_str());
			m_Command = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[1].c_str();
			}
