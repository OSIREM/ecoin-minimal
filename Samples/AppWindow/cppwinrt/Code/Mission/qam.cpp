...
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-PortSHELL-VAR.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-PortSHELL-FUN.h"

#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-Includes.h"

#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-AI-Cortex.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-AI-Brain.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-AI-Heart.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-WE-WorldEffects.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-ITEM-ItemDef.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-Structs.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-CORE-Avatar.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-Prototypes.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-GlobalHeader.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-WE-PntSprite.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-WE-MeleeHits.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-WE-BulletSystem.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-WE-EnergyMatrix.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-WE-CloudControl.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-ENE-EnergyTypes.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-OBJ-ObjectTypes.h"
#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-AI-NNbot.h"

#include "../../../../Quantum Angel Universal 2018/Quantum Angel/Quantum Angel/QAGE Include/Q-NET-ProtoMap.h"
...
QAM::QAM(int f_QamID)
{
	...
	if(f_Int)
		...
		if(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size())
			{
			m_qaccountid = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[1].c_str());
			m_Name = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[2];
			m_Author = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[3];
			m_Description = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[4];
			m_Keywords = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[5];
			m_Features = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[6];
			m_FeaturedIndex = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[7].c_str());
			m_LevelCount = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[9].c_str());
			m_evalue = atof(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[8].c_str());
			m_ECN = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[8];
			}
	...
	int f_TerrainCount = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size();
	...
	if(f_TerrainCount > 0)
		...
		for(int f_Count = 0; f_Count < f_TerrainCount; f_Count++)
			{
			std::string f_String = g_Schedular[2]->m_Entries[f_GrentIndex]->at(f_Count)[0];
	...
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClear", (ecom_base::ControllerMethod_void)&QAM::acClear);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "ac_Save", (ecom_base::ControllerMethod_void)&QAM::ac_Save);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "ac_LoadAll", (ecom_base::ControllerMethod_void)&QAM::ac_LoadAll);
	ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "ac_SaveAll", (ecom_base::ControllerMethod_void)&QAM::ac_SaveAll);
}
...
QAM* ag_LoadBrief(int f_QamID)
{
	...
	if(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size())
		{
		f_QAM->m_qaccountid = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[1].c_str());
		f_QAM->m_Name = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[2];
		f_QAM->m_Author = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[3];
		f_QAM->m_Description = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[4];
		f_QAM->m_Keywords = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[5];
		f_QAM->m_Features = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[6];
		f_QAM->m_FeaturedIndex = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[7].c_str());
		f_QAM->m_LevelCount = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[9].c_str());
		f_QAM->m_evalue = atof(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[8].c_str());
		f_QAM->m_ECN = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[8];
		}
...
void QAM::ac_LoadAll(void)
{
	...
	int f_TriggerCount = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size();
	...
	if(f_TriggerCount > 0)
		...
		for(int f_Count = 0; f_Count < f_TriggerCount; f_Count++)
			{
			qamTrigger* f_Trigger = new qamTrigger(atoi(g_Schedular[2]->m_Entries[f_GrentIndex]->at(f_Count)[0].c_str()));
	...
	int f_SpawnCount = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size();
	...
	if(f_SpawnCount > 0)
		...
		for(int f_Count = 0; f_Count < f_SpawnCount; f_Count++)
			{
			qamSpawn* f_Spawn = new qamSpawn(atoi(g_Schedular[2]->m_Entries[f_GrentIndex]->at(f_Count)[0].c_str()));
	...
	int f_EventCount = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size();
	...
	if(f_EventCount > 0)
		...
		for(int f_Count = 0; f_Count < f_EventCount; f_Count++)
			{
			qamEvent* f_Event = new qamEvent(atoi(g_Schedular[2]->m_Entries[f_GrentIndex]->at(f_Count)[0].c_str()));
...
void QAM::ac_Save(void)
{
	if(m_qamid == -1551)
		{
		/*char* f_qamidquery = new char[150];
		sprintf(f_qamidquery, "SELECT id, dated FROM qam ORDER BY dated DESC LIMIT 1");
		g_SafeL[2]->acSelectCommand(f_qamidquery, false, false);
		delete[] f_qamidquery;
		int f_QamidCount = g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->size();

		printf("ECN::Stat::: f_QamidCount %i\n", f_QamidCount);
		...
		if(f_QamidCount == 0)
			...
			{
			f_QamID = atoi(g_Schedular[2]->m_Entries[g_Schedular[2]->ac_GetEntry(g_Schedular[2]->m_Entries.size() - 1)]->at(0)[0].c_str());
			}*/
