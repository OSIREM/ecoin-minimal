...
namespace ecoin
{
	...
	class Variable : MuType
		...
		public:
			...
			~Variable() { acClear(); m_Action = MuAct_Finish; };
			...
			Variable(uint f_CodeType) : MuType(MuTYPE_Variable), m_CodeType(f_CodeType), m_PrecType(MuVar_Full)
				{
                m_Action = MuAct_Start;
				}
            ...
            std::shared_ptr<Variable> cloneVar(const std::shared_ptr<Variable> v)
				{
				return v;
				}
