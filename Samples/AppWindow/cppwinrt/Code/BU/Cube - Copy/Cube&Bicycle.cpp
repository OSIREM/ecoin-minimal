...
namespace Cube
{
	std::vector<Cube::CubeTransaction*> g_vec_Successfull_TX;
	std::shared_ptr<CubeHESH> g_TargetedHesh;
	...
	Cube::CubeHeshExporter* g_CubeHeshExport;
	...
	std::shared_ptr<CubeHESH> agGenerateHesh(void)
		...
		for(int f_Int = 0; f_Int < CUBE_HESH_SIZE; f_Int++)
			...
			Cube::agSet_offsetvec(Cube::Cube::float3(((Cube::random() - 0.5) * 5), ((Cube::random() - 0.5) * 5), ((Cube::random() - 0.5) * 5)));
...
ShareContainer* ag_SelectPeerGraph(int f_JobID, int f_ServLay, int f_ShareID, int f_ShareOffset, int f_PEER)
{
	...
	if(!g_SafeL[f_ServLay])
		...
		f_Int = g_SafeL[f_ServLay]->Connect("localhost", "root", "f", "gdb", 3306);
		}
...
void ag_UpdateOwnerBalance(std::string f_Owner, float f_Amount)
{
	...
	f_balupvalue = (char*)malloc(sizeof(char) * 1024);
	f_balupfield = (char*)malloc(sizeof(char) * 1024);
	f_balupwhere = (char*)malloc(sizeof(char) * 1024);

	sprintf(f_balupfield, "amt");
	sprintf(f_balupvalue, "%f", f_Amount);
	sprintf(f_balupwhere, "owner = '%s'", f_Owner.c_str());

	g_SafeL[0]->Update("address", (const char **)f_balupfield, (const char **)f_balupvalue, 1, (const char*)f_balupwhere, 2, false);
}
					...
					if((f_Transaction->m_txin[f_Jet]->m_vout == "Genesis Tracking") || (f_Transaction->m_txin[f_Jet]->m_vout.compare("GenesisTX") == 0))
						...
						if(f_RewardLCount != 1)
							...
							if(f_Reward != f_Transaction->m_reward)
								...
								{
								if(f_Reward <= 0.0f || f_Reward > 25.0f)
									{
									f_Verified = VERIFY_FAIL;
		...
		if(f_Transaction->m_fresh == true)
			...
			if(f_Verified == true)
				...
				if(f_Transaction->m_fresh == false)
					...
					f_txfailvalue = (char*)malloc(sizeof(char) * 1024);
					f_txfailfield = (char*)malloc(sizeof(char) * 1024);
					f_txfailwhere = (char*)malloc(sizeof(char) * 1024);

					sprintf(f_txfailfield, "status");
					sprintf(f_txfailvalue, "%i", -5);
					sprintf(f_txfailwhere, "id = %i", f_Transaction->m_txid);

					g_SafeL[0]->Update("transactions", (const char **)f_txfailfield, (const char **)f_txfailvalue, 1, (const char*)f_txfailwhere[0], 2, false);
...
CKey* ag_GenerateAddressCube(int f_Prefix)
{
		///////////////
	   // Research
	  // Entropy
	...
	while(true)
		...
		Cube::CubeCKey* f_CCKey = new Cube::CubeCKey(*key);
		...
		//Bitcoin Serialized PubKey 64 f_Char)
		...
		//Bitcoin Serialized ripem(sha256) PubKey 40
		...
		f_CCKey->m_str_Address = ((std::string)EncodeBase58((const std::vector<unsigned char> &)f_vec_UnsignedChar5)).c_str();
...
void Cube::CubeTransaction::acUpdateSync(void)
{
	...
	for(int f_s = 0; f_s < 6; f_s++)
		{
		delete g_syncvalue[f_s];
		delete g_syncfield[f_s];
		}

	delete g_txoutsyncwhere[0];

	for(int f_s2 = 0; f_s2 < 4; f_s2++)
		{
		delete g_txoutsyncvalue[f_s2];
		delete g_txoutsyncfield[f_s2];
		}
	...
	for(int f_Int = 0; f_Int < m_txout.size(); f_Int++)
		...
		delete g_txoutsyncvalue[0];
		delete g_txoutsyncvalue[1];
		delete g_txoutsyncvalue[2];
		delete g_txoutsyncvalue[3];
...
void Cube::CubeTransaction::acSynchronize(void)
{
	...
	if(f_SyncLCount == 0)
		{	//Unique Transaction Made ID = f_SyncUniqueID - txinsyncvalue
...
int ag_GenerateTransaction(std::string f_A, std::string f_B, float f_amt, Cube::CubeTransaction* f_tx)
	{
		///////////////
	   // Research
	  // Entropy
	...
	if(key)
		...
		f_tx = new CubeTransaction(f_A, f_B, f_amt);
...
#pragma message("add conditional fail on address not known")
		if(1)
			...
			if(f_amt > f_owneramt)
				...
				for(int f_Jet = 0; f_Jet < f_tx->m_txin.size(); f_Jet++)
					...
					sprintf(f_balupfield, "amt");
					sprintf(f_balupvalue, "%f", f_finalBalance);
					sprintf(f_balupwhere, "owner = '%s'", f_tx->m_txin[0]->m_vout.c_str());

					g_SafeL[0]->Update("address", (const char **)f_balupfield, (const char **)f_balupvalue, 1, (const char*)f_balupwhere, false, false);
					}
		...
		//ECN::successfully completed synchronisation of transaction
	...
	//ECN::transaction synchronisation failed
...
bool ag_addGenesisTracking(int f_gnid)
{
	...
	sprintf(f_Char, "%u%u%u%u", f_Info.Id().Data1, f_Info.Id().Data2, f_Info.Id().Data3, f_Info.Id().Data4);

	std::string Id = f_Char;
	...
	if(f_hdLCount == 0)
		...
		if(g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)][0][0].at(0) == '0')
			...
			{
			f_hlCount = g_Schedular[0]->m_Entries[g_Schedular[0]->ac_GetEntry(g_Schedular[0]->m_Entries.size() - 1)].size();
			}
