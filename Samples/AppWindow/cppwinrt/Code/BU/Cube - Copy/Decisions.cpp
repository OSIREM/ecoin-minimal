...
void DecisionManager::acLoad_ApplicationSpecificLegacy(void)
{
	...
	std::shared_ptr<BiDecisionA> f_ClickContractAnswer = std::make_shared<BiDecisionA>("Click to Change Contract Type", acLoadA_ContractToggle() + ":");
	std::shared_ptr<BiDecisionA> f_InsertBeginAnswer = std::make_shared<BiDecisionA>("Click to Insert Input Sense", acLoadA_InsertCom(ECOIN_CID_INPUTSENSE) + ":");

	 /////////////////////////////////////////////////////////////////////////
    // Shop Own Buy Full Grid Answers - Nothing Entered - Nothing Selected //
	std::shared_ptr<BiDecisionA> f_LoadOwnAnswer = std::make_shared<BiDecisionA>("Click to Load Own E-Grid", acLoadA_ShareShop(ECOIN_SHARESHOP_OWNCOM) + ":");
	std::shared_ptr<BiDecisionA> f_BuySearchShopWholeKitsAnswer = std::make_shared<BiDecisionA>("Click to Search Whole Buy Kits", acLoadA_ShareShop(ECOIN_SHARESHOP_GROUP_KITS) + ":");

	 ////////////////////////////////////////////////////////////
	// Generic Decisions - Nothing Entered - Nothing Selected //
	std::shared_ptr<BiDecisionQ> f_EmptyDecision = std::make_shared<BiDecisionQ>();
	...
	std::shared_ptr<BiDecisionQ> f_Decision1 = std::make_shared<BiDecisionQ>();
	...
	std::shared_ptr<BiDecisionQ> f_DecisionOne = std::make_shared<BiDecisionQ>();
	...
	if(m_Wheel->m_vec_Selection_com.size() > 0)
		...
		f_CopyAnswer = std::make_shared<BiDecisionA>("Click to Copy Items", f_AnswerString + ":");
		...
		f_PasteAnswer = std::make_shared<BiDecisionA>("Click to Paste Items", f_AnswerString + ":");
		...
		f_DeleteAnswer = std::make_shared<BiDecisionA>("Click to Delete Items", f_AnswerString + ":");
		...
		f_CutAnswer = std::make_shared<BiDecisionA>("Click to Cut Items", f_AnswerString + ":");
		}

	 //////////////////////////////////////////////
    // Generic Decisions - One >= ECOM Selected //
	std::shared_ptr<BiDecisionQ> f_DecisionEdit = std::make_shared<BiDecisionQ>();
	...
	if((m_Wheel->m_vec_Selection_com.size() >= 2) && (m_Wheel->m_vec_Selection_output.size() >= 1) && (m_Wheel->m_vec_Selection_input.size() >= 1))
		{
		std::shared_ptr<BiDecisionA> f_StandardWireAnswer = std::make_shared<BiDecisionA>("Click to Standard Wire Two Components", acLoadA_WireStandard(m_Wheel->m_vec_Selection_com[0], m_Wheel->m_vec_Selection_com[1], m_Wheel->m_vec_Selection_output[0], m_Wheel->m_vec_Selection_input[0]) + ":");

		 ////////////////////////////////////////
		// Wire Decisions - Two ECOM Selected //
		std::shared_ptr<BiDecisionQ> f_DecisionStandardWire = std::make_shared<BiDecisionQ>();
...
std::string DecisionManager::acLoadA_QamByID(int f_QamID, time_t f_Trial)
{
	char* f_CID_Char = ag_ClearChar(256);
...
bool BiDecisionA::acApply(void)
{
	...
	while(f_ScanFull == true)
		...
		if(f_funcom[0].compare("makcom") == 0)
			{
			std::shared_ptr<Qcom> f_com = std::make_shared<Qcom>(atoi(f_funcomID[0].c_str()));
