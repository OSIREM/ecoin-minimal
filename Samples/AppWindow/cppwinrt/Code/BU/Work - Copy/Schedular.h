...
namespace ecoin
{
	...
	class Schedular
		...
		public:
			...
			bool acSend_Query(char* f_Char, int f_UI, bool f_End);
			bool acSend_Query_Angel(char* f_Char, int f_UI, bool f_End);
			bool acSend_Query_PeerGraph(char* f_Char, int f_UI, bool f_End);
			bool acSend_Query_PeerGraphSelect(char* f_Char, int f_UI, bool f_End, int f_ShareID, int f_ShareOffset, int f_PEER);
			bool acSend_Query_GlobalStorageWealth(char* f_Char);
			bool acSend_Query_CMD(char* f_Char, int f_UI, bool f_End, bool f_Update = false);
			bool acSend_Query_CMD_Angel(char* f_Char, int f_UI, bool f_End, bool f_Update = false);

			int acNet_Client(void);
			...
			SOCKET m_ServerListenSocket;
			SocketSet* m_ClientSS;

			std::vector<std::vector<std::vector<std::string>>> m_Entries;
			std::vector<int> m_Entrant;
	};

	extern Schedular* g_Schedular[3];
