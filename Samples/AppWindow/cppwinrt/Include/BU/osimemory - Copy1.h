...
bool g_MutexLocked = false;
int g_LockedForCount = 0;
...
#if 0
template<class T>
T ag_VisitVector(std::vector<T> f_Vector, int f_IDX)
{
    return f_Vector[f_IDX];
}

template<class T>
int ag_SizeOfVector(std::vector<T> f_Vec)
{
    return f_Vec.size();
}
#endif
...
int ag_Pull_GID(void)
{
    ...
    if((g_vec_MID[0] < 0) || (g_vec_MID[0] >= g_vec_ref_MID.size()))
        {
        throw;
        }
#else
    ...
    int f_Res = g_vec_MID[0];

    for(int f_XY = 0; f_XY < (g_vec_MID.size() - 1); f_XY++)
        {
        g_vec_MID[f_XY] = g_vec_MID[f_XY + 1];
        }
...
template<class T>
void ag_MIDXY_DeleteReorder(int f_MIDX, int f_MIDY)
{
    ag_LockMutex();

    for(int f_XY = f_MIDY; f_XY < (g_vecvec_ref_SharedPtr<T>[f_MIDX].size() - 1); f_XY++)
        {
        g_vecvec_ref_SharedPtr<T>[f_MIDX][f_XY] = g_vecvec_ref_SharedPtr<T>[f_MIDX][f_XY + 1];
        g_vec_ref_MID[f_MIDX][f_XY] = g_vec_ref_MID[f_MIDX][f_XY + 1];
        }

#if 0 //conditionally reorder the typed main vector of vector
    if(g_vecvec_ref_SharedPtr<T>[f_GIDX].size() <= 0)
        {
        //...
        }
#endif

    g_vecvec_ref_SharedPtr<T>[f_MIDX].pop_back();
    g_vec_ref_MID[f_MIDX].pop_back();
...
public:
    ...
    void _Decwref() noexcept
        ...
        if(_MT_DECR(_Weaks) == 0)
            ...
            if((m_MIDX >= 0) && (m_MIDY >= 0))
                {
                ag_MIDXY_DeleteReorder<static_cast(g_vec_ref_TypeID[m_MIDX])>(m_MIDX, m_MIDY);
                }
            }
        }
...
private:
    ...
    template <class _Ty0, class... _Types>
    friend shared_ptr<_Ty0> make_shared_oage(_Types&&... _Args);
...
template<class T>
int ag_PlaceVecVec(std::shared_ptr<T> f_Smt_ptr)
{
    std::string f_TypeName = typeid(f_Smt_ptr).name();

    ag_LockMutex();

    //T_size?????
    for(int f_Jet = 0; f_Jet < g_vecvec_ref_SharedPtr<T>.size(); f_Jet++)
        {
        std::string f_Vec_ClassName = typeid(g_vecvec_ref_SharedPtr<T>[f_Jet][0]).name();

        if(f_Vec_ClassName.compare(f_TypeName) == 0)
            {
            //win conditions save shared_ptr
            g_vecvec_ref_SharedPtr<T>[f_Jet].push_back(f_Smt_ptr);

            return f_Jet;
            }
        }

    //else save new vector
    std::vector<T> f_NewClassVector;

    f_NewClassVector.clear();
    f_NewClassVector.push_back(f_Smt_ptr);

    //alternate win conditions push one x slot
    g_vecvec_ref_SharedPtr<T>.push_back(f_NewClassVector);
    g_vec_ref_TypeID.push_back(f_TypeName);

    ag_UnLockMutex();

    //T_size?????
    return g_vecvec_ref_SharedPtr<T>.size() - 1;
}
...
std::string ag_AcceptObjectName(void)
{
    ...
    if(g_VectorObjectName.size() > 0)
        {
        std::string f_PulledObjectName = g_VectorObjectName[0];

        for(int f_Jet = 0; f_Jet < (g_VectorObjectName.size() - 1); f_Jet++)
            {
            g_VectorObjectName[f_Jet] = g_VectorObjectName[f_Jet + 1];
            }

        g_VectorObjectName.pop_back();
...
void ag_StatusLive(std::string f_Label)
{
    ag_LockMutex();

    g_VectorObjectName.push_back(f_Label);
...
shared_ptr<_Ty>
#endif // _HAS_CXX20
make_shared_oage(_Types&&... _Args) { // make a shared_ptr to non-array object
...
// OSI FUNCTION TEMPLATE make_shared
template <class _Ty, class... _Types>
_NODISCARD
#if _HAS_CXX20
enable_if_t<!is_array_v<_Ty>, shared_ptr<_Ty>>
...
make_shared(_Types&&... _Args) // make a shared_ptr to non-array object
{
    ...
    std::shared_ptr<void> f_Ptr = std::static_pointer_cast<void>(_Ret);

    int f_MIDX = ag_PlaceVecVec<_Ty>(_Ret);

    if(f_MIDX < 0)
        {
        throw; //dev throw wrong
        }

    _Ret._Rep->m_MIDX = f_MIDX;
    _Ret._Rep->m_MIDY = g_vecvec_ref_SharedPtr<_Ty>[f_MIDX].size();

    if(f_MIDX >= g_vecvec_ref_ObjectName.size())
        {
        std::vector<std::string> f_VecStr;

        f_VecStr.clear();
        f_VecStr.push_back(ag_AcceptObjectName());

        g_vecvec_ref_ObjectName.push_back(f_VecStr);
        }
    else
        {
        g_vecvec_ref_ObjectName[f_MIDX].push_back(ag_AcceptObjectName());
        }
