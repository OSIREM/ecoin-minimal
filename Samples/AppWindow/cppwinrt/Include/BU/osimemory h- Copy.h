...
public:
    ...
    //osirem reflection xy index pod
#ifdef ECOIN_OSI_MEMORY
    std::shared_ptr<MID_EscapePod> m_EscapePod;
#endif
...
#ifdef ECOIN_OSI_MEMORY
            if((m_EscapePod->m_MIDX >= 0) && (m_EscapePod->m_MIDY >= 0))
                {
                Cube::ag_MIDXY_DeleteReorder(m_EscapePod->m_MIDX, m_EscapePod->m_MIDY);
                }
#endif
            }
        }
...
extern std::shared_ptr<MID_Manager> g_MID_Manager;

   ////////////////////////////////
  //osi escape pod manager beam
 //
//
class MID_ManagementBeam
{
public:
    MID_ManagementBeam(void)
        {
        acClear();
        }

    MID_ManagementBeam(std::shared_ptr<MID_EscapePod> f_Pod)
        {
        acClear();

        m_vec_EscapePod.push_back(f_Pod);

        m_IDX = g_MID_Manager->acCount_Beams() - 1;
        }

    MID_ManagementBeam(std::shared_ptr<MID_EscapePod> f_Pod, int f_MIDX)
        {
        acClear();
    ...
    void acClear(void)
        ...
        m_vec_EscapePod.clear();
        }

    int acCount_Pods(void)
        {
        return m_vec_EscapePod.size();
        }

    void acPush(std::shared_ptr<MID_EscapePod> f_Pod)
        {
        m_vec_EscapePod.push_back(f_Pod);
        }

    std::shared_ptr<MID_EscapePod> acPull_Pod(int f_MIDY)
        {
        return m_vec_EscapePod[f_MIDY];
        }

    void acDecX(void)
        {
        for(int f_Jet = 0; f_Jet < acCount_Pods() - 1; f_Jet++)
            {
            m_vec_EscapePod[f_Jet]->acDecX();
            }

        m_IDX--;
        }

    void acDecY(int f_MIDY)
        {
        for(int f_Jet = f_MIDY; f_Jet < acCount_Pods() - 1; f_Jet++)
            {
            m_vec_EscapePod[f_Jet]->acDecY();
            }

        m_vec_EscapePod.pop_back();
        }

    int m_IDX;

    std::vector<std::shared_ptr<MID_EscapePod>> m_vec_EscapePod;
};
...
public:
    ...
    void acPush(shared_ptr<MID_ManagementBeam> f_PodBeam)
        {
        m_vec_PodBeam.push_back(f_PodBeam);
        }

    shared_ptr<MID_ManagementBeam> acPull_Beam(int f_MIDX)
        {
        return m_vec_PodBeam[f_MIDX];
        }

    int acCount_Beams(void)
        {
        return m_vec_PodBeam.size();
        }

    std::vector<std::shared_ptr<MID_ManagementBeam>> m_vec_PodBeam;
};

//osirem reflection
extern int ag_PlaceVecVec(boost::any f_Smt_ptr);
extern bool ag_ConnectSingle(std::string f_NameString, std::shared_ptr<MID_EscapePod> f_Pod);
extern std::vector<std::string> g_vec_single_order_Name;
extern std::vector<std::string> g_vec_singles_Name;
extern std::vector<std::shared_ptr<MID_EscapePod>> g_singles_midpod;
...
#ifdef ECOIN_OSI_MEMORY_I01
    std::shared_ptr<void> f_Ptr = std::static_pointer_cast<void>(_Ret);

    std::string f_typename = typeid(shared_ptr<_Ty>).name();

    int f_newMIDX = ag_PlaceVecVec(_Ret);
