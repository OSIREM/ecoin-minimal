    ...
    void _Decwref() noexcept
        ...
        if(_MT_DECR(_Weaks) == 0)
            ...
            if((m_MIDX >= 0) && (m_MIDY >= 0))
                {
                ag_MIDXY_DeleteReorder(m_MIDX, m_MIDY);
                }
#endif
            }
        }
...
//osirem reflection
extern int ag_PlaceVecVec(std::shared_ptr<void> f_Smt_ptr, std::string f_TypeString);
extern void ag_ConnectSingle(std::string f_NameString, std::shared_ptr<void> f_Ptr);
extern std::vector<std::shared_ptr<void>> g_singles;
...
make_shared_reflect(_Types&&... _Args) // make a shared_ptr to non-array object
{
    ...
    int f_MIDX = ag_PlaceVecVec(_Ret, typeid(shared_ptr<_Ty>).name());
    ...
    _Ret._Rep->m_MIDY = g_vecvec_ref_SharedPtr[f_MIDX].size();
    ...
    if(f_MIDX >= g_vecvec_ref_ObjectName.size())
        ...
        f_VecStr.clear();
        f_VecStr.push_back(f_AOMStr);

        ag_ConnectSingle(f_AOMStr, f_Ptr);
        ...
        g_vecvec_ref_ObjectName[f_MIDX].push_back(f_AOMStr);
        }
