...
make_shared_reflect(_Types&&... _Args) // make a shared_ptr to non-array object
{
    ...
    ag_LockMutex();
     ...
     ////////////////////
    //Current Count UP//
    if(g_vec_ecomRefCount.empty() == false)
        {
        if(g_vec_ecomRefCount.find(f_CountString) == g_vec_ecomRefCount.end())
            {
            g_vec_ecomRefCount.emplace(f_CountString, 1);

            f_ecomRefBase->m_StackCount = 0;
            }
        else
            {
            f_ecomRefBase->m_StackCount = g_vec_ecomRefCount[f_CountString];

            g_vec_ecomRefCount[f_CountString]++;
            }
        }
    else
        {
    ...
    if(g_vec_ecomRefBase.empty() == false)
        {
        if(g_vec_ecomRefBase.find(f_CountString) == g_vec_ecomRefBase.end())
            {
            g_vec_ecomRefBase.emplace(f_CountString, f_ecomRefBase);
            ...
            while(g_vec_ecomRefBase.find(f_CountString) != g_vec_ecomRefBase.end())
                {
