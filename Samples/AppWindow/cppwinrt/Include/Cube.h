...
#define ECOIN_CLS_WIRE_BIRTH 7
	...
	class BiColor : public ecom_base
		{
		public:
			BiColor()
				{
...
#ifdef ECOIN_OSI_MEMORY
				ac_Push(m_U, f_ref_CurrentPath + g_ref_Div + "m_U");
				ac_Push(m_V, f_ref_CurrentPath + g_ref_Div + "m_V");
#endif
				}
	...
	class BicycleContainer : public ecom_base
		...
		public:
			BicycleContainer()
				...
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "avStage", (ecom_base::ControllerMethod_void)&BicycleContainer::avStage);
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "ac_IsOK", (ecom_base::ControllerMethod_void)&BicycleContainer::ac_IsOK);
#endif
				}
...
#if 0
				ac_Push((std::vector<unsigned char>)m_sig, f_ref_CurrentPath + g_ref_Div + "m_sig");
#endif

				ac_Push(m_sigstr, f_ref_CurrentPath + g_ref_Div + "m_sigstr");
				ac_Push(m_amt, f_ref_CurrentPath + g_ref_Div + "m_amt");

				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClear", (ecom_base::ControllerMethod_void)&CubeTXIN::acClear);
#endif
				}
	...
	class CubeTransaction : public ecom_base
		...
		public:
			CubeTransaction()
				...
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acHash", (ecom_base::ControllerMethod_void)&CubeTransaction::acHash);
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acSynchronize", (ecom_base::ControllerMethod_void)&CubeTransaction::acSynchronize);
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acUpdateSync", (ecom_base::ControllerMethod_void)&CubeTransaction::acUpdateSync);
#endif
				}
	...
	class CubeHASH_originstring : public ecom_base
		...
		public:
			CubeHASH_originstring()
				...
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClear", (ecom_base::ControllerMethod_void)&CubeHASH_originstring::acClear);
#endif
				};
	...
	class BiVector : public ecom_base
		{
		public:
			BiVector()
				{
				std::string f_ref_CurrentPath = "BiVector::BiVector";
				m_X = 0.0f; m_Y = 0.0f; m_Z = 0.0f;
				ac_Push(m_X, f_ref_CurrentPath + g_ref_Div + "m_X"); ac_Push(m_Y, f_ref_CurrentPath + g_ref_Div + "m_Y"); ac_Push(m_Z, f_ref_CurrentPath + g_ref_Div + "m_Z");
				}
	...
	class Index : public ecom_base
		{
		public:
			Index()
				{
				std::string f_ref_CurrentPath = g_ref_global_start + "Index::Index";
				ag_StatusLive(f_ref_CurrentPath, "m_Dir");
				m_FaceNormal = std::make_shared_reflect<BiVector>(0.0f);
				m_A = 0;
				m_B = 0;
				m_C = 0;
	...
	class BiPlane : public ecom_base
		...
		public:
			BiPlane()
				...
				ag_StatusLive(f_ref_CurrentPath, "m_Dir");
				m_Dir = std::make_shared_reflect<BiVector>(0.0f);
	...
	class CubeBicycle : public ecom_base
		...
		public:
			CubeBicycle()
				...
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "acClear", (ecom_base::ControllerMethod_void)&CubeBicycle::acClear);
#endif
				};
	...
	class CubeHESH : public ecom_base
		...
		public:
			CubeHESH()
				...
				ac_PushFunction(f_ref_CurrentPath + g_ref_fun_Div + "ac_IsOK", (ecom_base::ControllerMethod_void)&CubeHESH::ac_IsOK);
				}
	...
	extern std::shared_ptr<Cube::CubeTransaction> ag_getTX(int f_txid, int f_txoutid = -1);

	extern std::shared_ptr<Cube::CubeTransaction> ag_getTX(int f_ShareID, float f_CostCPS);

	extern bool ag_VerifyTXIN(std::shared_ptr<CubeTXIN> f_txin, float f_amount);

	extern std::shared_ptr<Cube::CubeTransaction> ag_get_TXfromvout(std::string f_vout);

	extern void ag_Upload_Procedures(bool f_ResetProc = false);

	extern float ag_CalcOwnerGenesisTX(std::string f_Owner, int f_ShareID, std::shared_ptr<Cube::CubeTransaction> f_TX);

	extern bool ag_Verify_TX_ReHash_VerifySigniture(std::shared_ptr<Cube::CubeTransaction> f_TX);

	extern bool ag_Verify_PublicSigniture(std::string f_PubKey, std::string f_Hash, std::string f_Sign);

	extern float ag_OwnerBalance(std::string f_owner);

	extern void ag_UpdateOwnerBalance(std::string f_Owner, float f_Amount);

	extern std::shared_ptr<Cube::ShareContainerExt> ag_SelectPeerGraphExt(int f_JobID, int f_ServLay, int f_ShareID, int f_ShareOffset, int f_PEER, bool f_MainMutex = false);

	extern bool ag_makeGenesisCoin(void);

	extern bool ag_addGenesisTracking(int f_gnid, int f_blocktype, int f_elementinc = 0);

	extern int ag_GenerateTransaction(std::string f_A, std::string f_B, float f_amt, std::shared_ptr<CubeTransaction> f_tx = nullptr);

	extern std::shared_ptr<Cube::CubeResults> ag_GatherTransactions(int f_JobID, int f_IDX);

	extern std::string ag_GatherAllReferences(int f_JobID, int f_IDX);

	extern std::string ag_cont_VerifySingleTransaction(std::shared_ptr<CubeTransaction> f_Transaction, bool f_initialVerified);

	extern std::string ag_cont_VerifyResult(std::shared_ptr<CubeTransaction> f_Transaction, bool f_initialVerified);
